...
export default function CoverLettersPage() {
  ...
  const [clDocs, setClDocs] = useState<GeneratedDoc[]>([])
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white overflow-hidden">
      <div className="px-8 py-6 border-b border-gray-200 shrink-0">
        <h1 className="font-heading text-2xl font-bold text-[#0A0A0A]">Cover Letters</h1>
        <p className="font-sans text-sm text-gray-500 mt-1">Your original cover letter and all Ghost-written variants.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 bg-gray-50">
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          <Accordion
            ...
            subtitle="Parsed text from your uploaded cover letter template"
            icon={<FileText size={18} />}
            defaultOpen
          >
            {candidateProfile.rawCoverLetterText ? (
              ...
              <div className="py-8 text-center">
                <p className="font-sans text-sm text-gray-400">
                  No cover letter uploaded yet. Complete onboarding or upload one from your profile settings.
                </p>
              </div>
            )}
          </Accordion>
          ...
          <Accordion
            ...
            subtitle={`${clDocs.length} document${clDocs.length !== 1 ? "s" : ""} generated`}
            icon={<Layers size={18} />}
            defaultOpen
          >
            {isLoading ? (
              ...
              <div className="py-10 text-center">
                <p className="font-sans text-sm text-gray-400">
                  No tailored cover letters yet. Approve a job from the Job Tracker to generate your first one.
                </p>
              </div>
            ) : (
              <div className="flex flex-col gap-4">
                {clDocs.map((doc) => <TailoredDocCard key={doc.id} doc={doc} />)}
              </div>
            )}
          </Accordion>
